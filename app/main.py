...
def handle_default_error(
    exc: Exception, status_code: int, headers: Optional[Dict] = None
) -> JSONResponse:
    return JSONResponse(
...
async def pydantic_validation_error_handler(
    request: Request, exc: pydantic.error_wrappers.ValidationError  # noqa
) -> JSONResponse:
    return JSONResponse(
...
async def test_job() -> None:
    ...
    async with wrap_session() as session:
        ...
        orders_to_remove = await order_service.read_many(
            status=schemas.OrderStatus.TO_REMOVE
        )
        for order in orders_to_remove:
            if order.history[-1].created_at + timeout < datetime.now(pytz.UTC):
                ...
                await order_service.update(
                    id=str(order.id),
                    **jsonable_encoder(
                        schemas.OrderUpdate(status=schemas.OrderStatus.REMOVED),
                        exclude_none=True,
                    ),
                )
