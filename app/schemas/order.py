...
def raise_order_status_update(
    user: User, old_status: OrderStatus, new_status: OrderStatus
) -> None:
...
def raise_ready_order_update(new_status: OrderStatus, order: entities.Order) -> None:
    ...
    for param in order.order_type.params:
        if param.required and (
            param.id not in order_params_types.keys()
            or order_params_types[param.id] is None
        ):
...
def raise_accepted_order_update(
    new_status: OrderStatus, child_orders: Sequence[entities.Order]
) -> None:
    if new_status != OrderStatus.ACCEPTED:
        return
    logger.info(child_orders)
    for child_order in child_orders:
        if child_order.status not in [
            OrderStatus.ACCEPTED,
            OrderStatus.TO_REMOVE,
            OrderStatus.REMOVED,
        ]:
